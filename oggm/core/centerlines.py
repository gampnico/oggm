...
def compute_centerlines(gdir, heads=None):
    ...
    # Write the data
    gdir.write_npz(cls, 'centerlines')
...
def compute_downstream_line(gdir):
    ...
    try:
        ...
        p = gdir.read_npz('centerlines')[-1].tail
    ...
    gdir.write_npz(out, 'downstream_line')
...
def compute_downstream_bedshape(gdir):
    ...
    cl = gdir.read_npz('downstream_line')['downstream_line']
...
def catchment_area(gdir):
    ...
    if len(cls) == 1:
        ...
        gdir.write_npz(geom, 'geometries')
        return
...
def catchment_intersections(gdir):
    ...
    catchment_indices = gdir.read_npz('geometries')['catchment_indices']
...
def catchment_width_geom(gdir):
    ...
    # variables
    flowlines = gdir.read_npz('inversion_flowlines')
    catchment_indices = gdir.read_npz('geometries')['catchment_indices']
    ...
    # Overwrite
    gdir.write_npz(flowlines, 'inversion_flowlines')
...
def intersect_downstream_lines(gdir, candidates=None):
    ...
    dline = gdir.read_npz('downstream_line')['full_line']
    crs = gdir.grid
    ...
    for trib in candidates:
        ...
        _dline = trib.read_npz('downstream_line')['full_line']
        _crs = trib.grid
