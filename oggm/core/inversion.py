...
def prepare_for_inversion(gdir,
    ...
    # variables
    fls = gdir.read_npz('inversion_flowlines', filesuffix=input_filesuffix)
    ...
    # Write out
    gdir.write_npz(towrite, 'inversion_input', filesuffix=output_filesuffix)
...
def mass_conservation_inversion(gdir, settings_filesuffix='',
    ...
    cls = gdir.read_npz('inversion_input', filesuffix=input_filesuffix)
    for cl in cls:
    ...
    if write:
        gdir.write_npz(cls, 'inversion_output', filesuffix=output_filesuffix)
...
def filter_inversion_output(gdir, settings_filesuffix='',
    ...
    if gdir.is_tidewater:
        ...
        cls = gdir.read_npz('inversion_output', filesuffix=input_filesuffix)
    ...
    dic_ds = gdir.read_npz('downstream_line')
    cls = gdir.read_npz('inversion_output', filesuffix=input_filesuffix)
    cl = cls[-1]
...
def calving_flux_from_depth(gdir, input_filesuffix='',
    ...
    # Read necessary data
    fl = gdir.read_npz('inversion_flowlines', filesuffix=input_filesuffix)[-1]
    ...
    # Calving formula
    if thick is None:
        cl = gdir.read_npz('inversion_output', filesuffix=input_filesuffix)[-1]
        thick = cl['thick'][-1]
...
def find_inversion_calving_from_any_mb(gdir, settings_filesuffix='',
    ...
    cls = gdir.read_npz('inversion_input', filesuffix=output_filesuffix)[-1]
    ...
    fl = gdir.read_npz('inversion_flowlines', filesuffix=output_filesuffix)[-1]
